...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.password) {
      ...
      newErrors.password = 'Requires 8+ chars, 1 uppercase, 1 number, & 1 special char';
    }
    ...
    setLoading(true);
    const result = await registerAction({ name: formData.name, email: formData.email, password: formData.password });
    setLoading(false);
    ...
    pushToast({ title: 'Workspace Provisioned', type: 'success' });
      ...
      <div className="w-full max-w-5xl lg:scale-95 origin-center z-10 grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-7 lg:gap-7 relative my-auto">
        ...
        >
          ...
          <motion.div variants={defaultFadeIn} className="relative z-10">
            ...
            <h1 className="text-2xl xl:text-3xl font-display font-medium leading-[1.1] tracking-tight mb-6 mt-4">
              Provision Your <br/>
              <span className="text-gradient-brand font-bold">Intelligence Terminal.</span>
            </h1>
            <p className="text-slate-400 text-base max-w-[90%] leading-relaxed">
              Create your secure workspace to instantly begin ingesting, processing, and querying massive media datasets.
            </p>
          </motion.div>
          ...
          <motion.div variants={defaultFadeIn} className="relative z-10 pt-8 border-t border-white/10 flex items-center space-x-3">
            ...
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">Awaiting Registration</span>
          </motion.div>
        </motion.div>
        ...
        >
          ...
          <div className="mb-6 text-center lg:text-left">
            <h2 className="text-2xl font-display font-semibold mb-2">Request Access</h2>
            <p className="text-slate-400 font-medium text-sm sm:text-base">Initialize your administrator credentials.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            >
              ...
              <div className="relative w-full bg-brand text-white py-2.5 px-6 rounded-2xl font-bold tracking-widest uppercase flex items-center justify-center space-x-2 border border-brand-light/30 shadow-lg group-hover:bg-brand-light transition-all disabled:opacity-50">
                <span>{loading ? 'Initializing...' : 'Sign Up'}</span>
